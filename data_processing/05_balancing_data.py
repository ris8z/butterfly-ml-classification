...
def get_random_row(data_frame):
    return data_frame.sample(1).iloc[0] # sample give u a set of 1 element ln, iloc pick the first one
...
def load_and_resize_image(file_name, folder_path, height, width):
    ...
    return (img_resized * 255).astype(np.uint8)  # change values in uint8
...
def generate_new_images(needed_image_n, df_label, n, label, result_csv):
    ...
    for i in range(needed_image_n):
        ...
        if origianl_img.ndim == 2:  # Controllo per immagini grayscale
            print("e' utile over oh")
